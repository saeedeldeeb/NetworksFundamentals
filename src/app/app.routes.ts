import { Routes } from '@angular/router';

export const routes: Routes = [
  {
    path: '',
    loadComponent: () => import('./pages/home/home').then((m) => m.Home),
  },
  {
    path: 'section/1',
    redirectTo: 'section/1/1',
  },
  {
    path: 'section/1/1',
    loadComponent: () => import('./pages/section1/lesson1/lesson1').then((m) => m.Section1Lesson1),
  },
  {
    path: 'section/1/2',
    loadComponent: () => import('./pages/section1/lesson2/lesson2').then((m) => m.Section1Lesson2),
  },
  {
    path: 'section/1/3',
    loadComponent: () => import('./pages/section1/lesson3/lesson3').then((m) => m.Section1Lesson3),
  },
  {
    path: 'section/2',
    redirectTo: 'section/2/1',
  },
  {
    path: 'section/2/1',
    loadComponent: () => import('./pages/section2/lesson1/lesson1').then((m) => m.Section2Lesson1),
  },
  {
    path: 'section/2/2',
    loadComponent: () => import('./pages/section2/lesson2/lesson2').then((m) => m.Section2Lesson2),
  },
  {
    path: 'section/2/3',
    loadComponent: () => import('./pages/section2/lesson3/lesson3').then((m) => m.Section2Lesson3),
  },
  {
    path: 'section/3',
    redirectTo: 'section/3/1',
  },
  {
    path: 'section/3/1',
    loadComponent: () => import('./pages/section3/lesson1/lesson1').then((m) => m.Section3Lesson1),
  },
  {
    path: 'section/3/2',
    loadComponent: () => import('./pages/section3/lesson2/lesson2').then((m) => m.Section3Lesson2),
  },
  {
    path: 'section/3/3',
    loadComponent: () => import('./pages/section3/lesson3/lesson3').then((m) => m.Section3Lesson3),
  },
  {
    path: 'section/3/4',
    loadComponent: () => import('./pages/section3/lesson4/lesson4').then((m) => m.Section3Lesson4),
  },
  {
    path: 'section/3/5',
    loadComponent: () => import('./pages/section3/lesson5/lesson5').then((m) => m.Section3Lesson5),
  },
  {
    path: 'section/3/6',
    loadComponent: () => import('./pages/section3/lesson6/lesson6').then((m) => m.Section3Lesson6),
  },
  {
    path: 'section/3/7',
    loadComponent: () => import('./pages/section3/lesson7/lesson7').then((m) => m.Section3Lesson7),
  },
  {
    path: 'section/4',
    redirectTo: 'section/4/1',
  },
  {
    path: 'section/4/1',
    loadComponent: () =>
      import('./pages/section4/lesson1/lesson1').then((m) => m.Section4Lesson1),
  },
  {
    path: 'section/4/2',
    loadComponent: () =>
      import('./pages/section4/lesson2/lesson2').then((m) => m.Section4Lesson2),
  },
  {
    path: 'section/4/3',
    loadComponent: () =>
      import('./pages/section4/lesson3/lesson3').then((m) => m.Section4Lesson3),
  },
  {
    path: 'section/4/4',
    loadComponent: () =>
      import('./pages/section4/lesson4/lesson4').then((m) => m.Section4Lesson4),
  },
  {
    path: 'section/4/5',
    loadComponent: () =>
      import('./pages/section4/lesson5/lesson5').then((m) => m.Section4Lesson5),
  },
  {
    path: 'section/4/6',
    loadComponent: () =>
      import('./pages/section4/lesson6/lesson6').then((m) => m.Section4Lesson6),
  },
  {
    path: 'section/5',
    redirectTo: 'section/5/1',
  },
  {
    path: 'section/5/1',
    loadComponent: () => import('./pages/section5/lesson1/lesson1').then((m) => m.Section5Lesson1),
  },
  {
    path: 'section/5/2',
    loadComponent: () => import('./pages/section5/lesson2/lesson2').then((m) => m.Section5Lesson2),
  },
  {
    path: 'section/5/3',
    loadComponent: () => import('./pages/section5/lesson3/lesson3').then((m) => m.Section5Lesson3),
  },
  {
    path: 'section/5/4',
    loadComponent: () => import('./pages/section5/lesson4/lesson4').then((m) => m.Section5Lesson4),
  },
  {
    path: 'section/5/5',
    loadComponent: () => import('./pages/section5/lesson5/lesson5').then((m) => m.Section5Lesson5),
  },
  {
    path: 'section/5/6',
    loadComponent: () => import('./pages/section5/lesson6/lesson6').then((m) => m.Section5Lesson6),
  },
  {
    path: 'section/5/7',
    loadComponent: () => import('./pages/section5/lesson7/lesson7').then((m) => m.Section5Lesson7),
  },
  {
    path: 'section/5/8',
    loadComponent: () => import('./pages/section5/lesson8/lesson8').then((m) => m.Section5Lesson8),
  },
  {
    path: 'section/5/9',
    loadComponent: () => import('./pages/section5/lesson9/lesson9').then((m) => m.Section5Lesson9),
  },
  {
    path: 'section/5/10',
    loadComponent: () =>
      import('./pages/section5/lesson10/lesson10').then((m) => m.Section5Lesson10),
  },
  {
    path: 'section/5/11',
    loadComponent: () =>
      import('./pages/section5/lesson11/lesson11').then((m) => m.Section5Lesson11),
  },
  {
    path: 'section/5/12',
    loadComponent: () =>
      import('./pages/section5/lesson12/lesson12').then((m) => m.Section5Lesson12),
  },
  {
    path: 'section/6',
    redirectTo: 'section/6/1',
  },
  {
    path: 'section/6/1',
    loadComponent: () => import('./pages/section6/lesson1/lesson1').then((m) => m.Section6Lesson1),
  },
  {
    path: 'section/6/2',
    loadComponent: () => import('./pages/section6/lesson2/lesson2').then((m) => m.Section6Lesson2),
  },
  {
    path: 'section/6/3',
    loadComponent: () => import('./pages/section6/lesson3/lesson3').then((m) => m.Section6Lesson3),
  },
  {
    path: 'section/6/4',
    loadComponent: () => import('./pages/section6/lesson4/lesson4').then((m) => m.Section6Lesson4),
  },
  {
    path: 'section/7',
    redirectTo: 'section/7/1',
  },
  {
    path: 'section/7/1',
    loadComponent: () => import('./pages/section7/lesson1/lesson1').then((m) => m.Section7Lesson1),
  },
  {
    path: 'section/7/2',
    loadComponent: () => import('./pages/section7/lesson2/lesson2').then((m) => m.Section7Lesson2),
  },
  {
    path: 'section/7/3',
    loadComponent: () => import('./pages/section7/lesson3/lesson3').then((m) => m.Section7Lesson3),
  },
  {
    path: 'section/7/4',
    loadComponent: () => import('./pages/section7/lesson4/lesson4').then((m) => m.Section7Lesson4),
  },
  {
    path: 'section/7/5',
    loadComponent: () => import('./pages/section7/lesson5/lesson5').then((m) => m.Section7Lesson5),
  },
  {
    path: 'section/7/6',
    loadComponent: () => import('./pages/section7/lesson6/lesson6').then((m) => m.Section7Lesson6),
  },
  {
    path: 'section/7/7',
    loadComponent: () => import('./pages/section7/lesson7/lesson7').then((m) => m.Section7Lesson7),
  },
  {
    path: 'section/7/8',
    loadComponent: () => import('./pages/section7/lesson8/lesson8').then((m) => m.Section7Lesson8),
  },
  {
    path: 'section/7/9',
    loadComponent: () => import('./pages/section7/lesson9/lesson9').then((m) => m.Section7Lesson9),
  },
  {
    path: 'section/7/10',
    loadComponent: () =>
      import('./pages/section7/lesson10/lesson10').then((m) => m.Section7Lesson10),
  },
  {
    path: 'section/7/11',
    loadComponent: () =>
      import('./pages/section7/lesson11/lesson11').then((m) => m.Section7Lesson11),
  },
  { path: '**', redirectTo: '' },
];
